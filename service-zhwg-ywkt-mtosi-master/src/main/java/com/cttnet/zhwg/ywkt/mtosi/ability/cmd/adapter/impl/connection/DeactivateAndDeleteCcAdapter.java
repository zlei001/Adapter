 ...
 * @since java 1.8
 */
public class DeactivateAndDeleteCcAdapter extends AbstractCmdAdapter {
    ...
    protected void createKvs() {
        ...
        if (FactoryName.ZTE == getFactoryName()
                && (aEndRefList.contains(Splits.SHARP) || zEndRefList.contains(Splits.SHARP))) {
        ...
        ccMap.put("isActive", MapUtils.getString(this.param, "I_ACTIVE", ""));
        ...
        ccMap.put("/E/B/d/c/c/ccType", MapUtils.getString(this.param, "I_CC_TYPE", ""));
        ccMap.put("aEndRefList", aEndRefList);
        ccMap.put("zEndRefList", zEndRefList);
        // 中兴、华为不添加附加信息
        if(!(FactoryName.ZTE == getFactoryName()) && !(FactoryName.HW == getFactoryName())){
            ccMap.put("/E/B/d/c/c/vendorExtensions", vendorExtensions);
...
//        ccMap.putAll(MtosiAtomicCapacityConfig.getInstn().getRequestthis.param(emsname));
    ...
    protected Cmd getCmd() {
        return Cmd.deactivateAndDeleteCrossConnection;
    }
    ...
    private void adjustParam() {
        ...
        //是否激活成功：成功、失败
        String isActive = MapUtils.getString(this.param, "I_ACTIVE", "");
        //转换枚举
        ...
        IsActive isActiveEnum = EnumUtils.getEnum(IsActive.class, isActive, "getCode");
        if (isActiveEnum != null) {
            this.param.put("I_ACTIVE", isActiveEnum.getName());
        }
    }
