 ...
 */
@Slf4j
public class ModifyBandwidthEthEos extends AbstractModifyAdapter {

    public ModifyBandwidthEthEos(ModifyBasicSvcRequestDTO modifyBasicSvcRequestDTO) {
    ...
    protected void modify() {
        ...
        ingressEgressBandwidthProfileA.setCIR(String.valueOf(modifyBasicSvcRequestDTO.getBandwidth()/1000*1024));
        ...
        ingressEgressBandwidthProfileZ.setCIR(String.valueOf(modifyBasicSvcRequestDTO.getBandwidth()/1000*1024));
        ...
        log.info("修改以太EOS业务入参：{}", JSON.toJSONString(requestOfCreateEthtSvc, SerializerFeature.NotWriteDefaultValue));
        StatusResponse response = null;
        try {
            response = new EthSvcClientImpl(this.actnMacEmsDTO).update(requestOfCreateEthtSvc);
            this.ok = response.isOk();
            if (!response.isOk()) {
                addErrInfo("修改以太EOS业务失败:" + response.getDesc(), null);
            } else {
                log.info("修改以太EOS业务成功");
            }
        } catch (Exception e) {
            log.error("修改以太EOS业务失败",e);
        }
