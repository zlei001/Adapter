...
import com.cttnet.ywkt.actn.capacity.adapter.impl.create.CreateEthEos;
import com.cttnet.ywkt.actn.capacity.adapter.impl.delete.DeleteEthSvc;
import com.cttnet.ywkt.actn.capacity.adapter.impl.modify.ModifyBandwidthEthEos;
import org.springframework.stereotype.Service;

/**
 * <pre>EOS业务Service</pre>
 *
 * @author zhangyaomin
 * @date 2020-06-20
 * @since jdk 1.8
...
public class EosSvcService {
    ...
    public String create(CreateBasicSvcRequestDTO request) {
        AbstractActnBaseAdapter adapter = new CreateEthEos(request);
        adapter.execute();
    ...
    public String updateBandwidth(ModifyBasicSvcRequestDTO modifyBasicSvcRequestDTO) {
        AbstractActnBaseAdapter adapter = new ModifyBandwidthEthEos(modifyBasicSvcRequestDTO);
        adapter.execute();
        Status status = adapter.getResponseOfAtomic();
        String result = status.isOk() ? "调用成功" : status.getDesc();
        return result;
    ...
    public EosSvcPrecomputedResponseDTO precomputed(EosSvcPrecomputedRequestDTO request) {
        return null;
    }
